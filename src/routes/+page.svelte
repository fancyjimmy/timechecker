<script lang="ts">
	...
	import { localStorageStore, type ModalSettings, modalStore, ProgressRadial } from '@skeletonlabs/skeleton';
	...
	{:else}
		...
		{#if currentTimer}
			<div class='container flex flex-col gap-3'>
				<Clock startTime={currentTimer.created_at} endTime={now} class='text-5xl font-bold text-center'>
				</Clock>
				<input type='text' class='input p-2' bind:value={text} placeholder='Was hast du gemacht?'/>
				<button class='btn variant-filled' on:click={() => stopTimer($user, text)}>Stop Timer</button>
			</div>
			{:else}
